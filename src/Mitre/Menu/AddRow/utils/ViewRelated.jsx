...
function ViewRelated({ _ids }) {
  ...
  const handle = (to) => {
    ...
    if (key === "l1_uc") ret.l1_uc_identifiers = selected("identifier");
      ...
      if (to === "l1_uc")
        ret.identifier = selected("l1_uc_identifiers")
          .map((i) => i.split(", "))
          .flat();
      else if (to === "l2_uc")
        ret.name = selected("l2_uc_identifiers")
          .map((i) => i.split(", "))
          .flat();
  ...
  return (
    <>
      {key === "l1_uc" && <But tos={["l2_uc", "l3_uc", "l4_uc"]} />}
      {key === "l2_uc" && <But tos={["l1_uc", "l3_uc", "l4_uc"]} />}
      {key === "l3_uc" && <But tos={["l1_uc", "l2_uc", "l4_uc"]} />}
      {key === "l4_uc" && <But tos={["l1_uc", "l2_uc", "l3_uc"]} />}
    </>
  );
}
